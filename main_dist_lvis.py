...
from src.helper_functions.get_data import get_data, get_data_dist_lvis
import numpy as np
...
parser.add_argument('--num-classes', default=1203)
...
parser.add_argument('--dist-url', default='file:///data/dongbowen/sharedfile', type=str,
...
def main():
    ...
    if args.model_path and not args.model_name.startswith("resnet"):  # make sure to load pretrained ImageNet model
        state = torch.load(args.model_path, map_location='cpu')
        filtered_dict = {k: v for k, v in state['state_dict'].items() if
                         (k in model.state_dict() and 'head.fc' not in k)}
        if args.rank == 0: print(filtered_dict.keys())
        model.load_state_dict(filtered_dict, strict=False)
    print('done\n')
    model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.local_rank], broadcast_buffers=False)
    # ---------------------------------------------------------------------------------
    train_loader, val_loader = get_data_dist_lvis(args)
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        with torch.no_grad():
            with autocast(enabled=args.amp):
                output_regular = Sig(model(input.to(args.rank, non_blocking=True)))#.cpu()
                output_ema = Sig(ema_model.module(input.to(args.rank, non_blocking=True)))#.cpu()
